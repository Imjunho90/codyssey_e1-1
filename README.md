...
## 3. 수행 체크리스트
### 터미널 · 권한
- [x] 현재 위치 확인 / 목록 확인(숨김 파일 포함)
- [x] 디렉토리 이동 · 생성 · 복사 · 이름변경 · 삭제
- [x] 빈 파일 생성(`touch`) 및 파일 내용 확인(`cat`)
- [x] 파일 권한 변경 전/후 비교
- [x] 디렉토리 권한 변경 전/후 비교
...
- [x] 컨테이너 실행 · 중지 · 목록 확인 (`docker run`, `stop`, `ps -a`)
- [x] 컨테이너 로그 확인 (`docker logs`)
- [x] 컨테이너 리소스 확인 (`docker stats`)

### 컨테이너 실행 실습
- [x] hello-world 실행
- [x] ubuntu 컨테이너 내부 진입 후 명령 수행
- [x] attach / exec 종료 동작 차이 관찰 및 정리

### 커스텀 이미지 · 네트워크 · 스토리지
- [x] Dockerfile 작성 및 커스텀 이미지 빌드
- [x] 커스텀 이미지 컨테이너 실행
- [x] 포트 매핑 접속 2회(8080 / 8081, 브라우저 화면 증거)
- [x] 바인드 마운트 호스트 변경 전/후 반영 확인
- [x] 볼륨 생성 · 연결 및 컨테이너 삭제 전/후 데이터 유지 확인

### Git · 문서
- [x] Git 사용자 정보 / 기본 브랜치 설정 (`git config --list`)
- [x] VSCode GitHub 로그인 및 저장소 연동
- [x] 로그 · 스크린샷 내 민감정보 마스킹
- [x] 트러블슈팅 2건 이상 기록
...
8081
![이미지2](img/port-mapping-8081.png)
